...
**Desafio:** PDFs imagem sem camada de texto.\
  Problema clássico: o PDF não tem texto extraível (é raster ou vetor “morto”), então regex/âncoras não funcionam. Sem detectar isso cedo, o pipeline fica “cego”, erra campos e pode até acionar LLM à toa. Precisamos distinguir rápido entre PDF textual e “desenhado”, evitando gastar tempo onde não há glyphs selecionáveis.
**Decisão:** **pipeline adaptativo**: tenta texto; se vazio, ativa OCR.\
  A execução sempre tenta extração textual nativa primeiro; se vier vazia (ou abaixo de um limiar de “densidade de glyphs”), o pipeline ativa OCR automaticamente. Nada de OCR “sempre ligado”: ele é custo variável e só entra quando há evidência forte de ausência de texto. O modo é configurável para cenários batch (forçar, desativar ou automático). \
...
**Desafio:** justificar escolhas para QA/evolução.\
  O problema não é apenas extrair; é explicar por que uma decisão foi tomada e onde o pipeline agregou (ou não) valor. Sem isso, QA vira arqueologia: difícil entender por que um PDF caiu em v2 hoje e, após um ajuste, migrou para v1 amanhã. Precisamos de rastros consistentes que permitam comparar execuções entre versões, medir impacto de normalizações e identificar rapidamente regressões ou regras frágeis.
**Decisão:** resposta sempre com **metadados de decisão**.\
  Toda resposta leva um bloco de metadados que torna a execução explicável e reprodutível. Junto do JSON “funcional”, retornamos a trilha de raciocínio do pipeline: qual layout venceu, qual foi a cobertura antes/depois das normalizações, qual pista heurística guiou a priorização e se houve fallback para LLM. Complementamos com meta (hash do PDF, versão do extrator, “sal” de versão e timestamp) para permitir auditoria pós-fato e comparações entre releases sem ambiguidade. \
**Solução:** `debug` inclui `layout_final`, `coverage.before/after`, `per_layout`, `detected_hint`, `llm_requested`.
  A execução expõe layout_final para evidenciar o candidato escolhido; coverage.before e coverage.after para quantificar o ganho real das normalizações/âncoras; per_layout para mostrar o placar completo (e revelar disputas apertadas que pedem desempate mais robusto); detected_hint para auditar a heurística inicial; e llm_requested como indicador binário de custo/latência quando o limiar não foi atingido. Com isso, QA consegue responder rapidamente “por que isso aconteceu?”, priorizar ajustes onde after não evolui, endurecer critérios quando per_layout mostra quase-empate e, principalmente, manter a taxa de LLM sob controle — LLM entra só quando precisa, nunca como uma desculpa invisível/sem critério.
...
**Desta forma** python -m venv venv sendo o segundo argumento venv o nome do ambiente.
